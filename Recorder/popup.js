...
async function initializePopup() {
    ...
    if (tab && tab.url) {
        ...
        if (detectedService) {
            // On supported sites (GMeet/Teams)
            await setCurrentService(detectedService);
            await checkMeetingStatus();
            showRecordingUI();
        } else {
            // On other sites - show features/how-to
            showFeaturesUI();
        }
    } else {
...
function showRecordingUI() {
    ...
    const serviceName = SERVICE_CONFIG[detectedService].name;
    const serviceIcon = SERVICE_CONFIG[detectedService].icon;
    serviceIndicator.innerHTML = `${serviceIcon} Recording: ${serviceName}`;
    
    // Update theme
    updateTheme(detectedService);
    
    // Update service-specific notes
    document.getElementById('gmeetNote').style.display = detectedService === 'gmeet' ? 'block' : 'none';
    document.getElementById('teamsNote').style.display = detectedService === 'teams' ? 'block' : 'none';
    
    console.log(`✅ Showing ${serviceName} recording UI`);
}
