// Teams-specific popup logic - Unified Popup Integration
let activeTabId;
let isRecording = false;
let autoRecordEnabled = false;

function initTeamsPopup(tabId) {
    console.log("🔄 Initializing Teams popup for tab:", tabId);
    activeTabId = tabId;
    
    // Initialize the Teams popup with the provided tab ID
    initializeTeamsPopup();    
}

function initializeTeamsPopup() {
    console.log("🔍 Teams popup initializing...");
...
function setupEventListeners() {
    ...
    document.getElementById("startBtn").addEventListener("click", async () => {
        ...
        try {
            ...
            chrome.tabs.create({
                url: chrome.runtime.getURL("platforms/teams/recorder.html"),
    ...
    document.getElementById("stopBtn").addEventListener("click", async () => {
        ...
        try {
            ...
            const tabs = await chrome.tabs.query({ url: chrome.runtime.getURL("platforms/teams/recorder.html") });
            if (tabs.length > 0) {
