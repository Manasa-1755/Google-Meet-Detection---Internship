// GMeet-specific popup logic - Unified Popup Integration
let activeTabId;
let isRecording = false;
let autoRecordEnabled = false;

function initGMeetPopup(tabId) {
    console.log("🔄 Initializing GMeet popup for tab:", tabId);
    activeTabId = tabId;
    
    // Initialize the GMeet popup with the provided tab ID
    initializeGMeetPopup();
}

function initializeGMeetPopup() {
    console.log("🔍 GMeet popup initializing...");

    // Initialize all components
    checkForFailedRecorders();
    checkRecordingStatus();
    checkAutoRecordPermission();
    verifyRecorderTabStatus();
    startUISyncChecker();

    // Check meeting status for the current tab
    if (activeTabId) {
...
async function closeAllRecorderTabs() {
    return new Promise((resolve) => {
        chrome.tabs.query({ url: chrome.runtime.getURL("platforms/gmeet/recorder.html") }, (tabs) => {
            if (tabs.length === 0) {
...
function setupEventListeners() {
    ...
    document.getElementById("startBtn").addEventListener("click", async () => {
        ...
        try {
            ...
            chrome.tabs.create({ url: chrome.runtime.getURL("platforms/gmeet/recorder.html"), active: false }, (tab) => {
                setTimeout(() => {
...
async function stopRecordingAndDownload() {
    ...
    try {
        const tabs = await chrome.tabs.query({ url: chrome.runtime.getURL("platforms/gmeet/recorder.html") });
        if (tabs.length > 0) {
